...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --border-rose: rgba(232,67,106,.2); --border-rose-strong: rgba(232,67,106,.35);
        ...
        ::-webkit-scrollbar-thumb { background: rgba(232,67,106,.3); border-radius: 4px; }

        /* Hero glow */
        .hero-glow-1 {
            position: absolute; width: 700px; height: 700px; border-radius: 50%;
            background: radial-gradient(circle, rgba(232,67,106,.09) 0%, transparent 70%);
        ...
        .hero-line {
            height: 1px;
            background: linear-gradient(90deg, transparent, var(--border-rose) 25%, var(--border-rose) 75%, transparent);
        }
        ...
        /* Hero name gradient */
        .hero-name-grad {
            background: linear-gradient(135deg, var(--text) 55%, rgba(232,67,106,.85));
            -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
        }
    </style>
</head>
...
<nav class="sticky top-0 z-50 h-16 flex items-center justify-between px-5 md:px-16"
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('perfiles') }}"
           class="inline-flex items-center gap-1.5 px-3 md:px-4 py-2 rounded-lg text-sm font-semibold no-underline transition-all"
        ...
        @auth
            @if(Auth::user()->isAdmin())
                <a href="{{ route('admin.usuarios.show', $perfil) }}"
                   class="inline-flex items-center px-3 md:px-4 py-2 rounded-lg text-sm font-semibold no-underline transition-all"
                   style="background:transparent;color:var(--text-2);border:1px solid var(--border)">
                    Admin
                </a>
            @endif
        ...
        @guest
            <a href="{{ route('register') }}"
               class="inline-flex items-center px-3 md:px-4 py-2 rounded-lg text-sm font-bold text-white no-underline transition-all"
...
{{-- ═══════════════════════════
     HERO
═══════════════════════════ --}}
<div class="relative overflow-hidden px-5 pt-10 pb-0 md:px-16 md:pt-12">
    ...
    <div class="relative z-10 max-w-5xl mx-auto
        ...
        <div class="relative flex justify-center md:justify-start shrink-0">
            ...
            <div class="w-24 h-24 md:w-32 md:h-32 rounded-[18px] md:rounded-[22px]
                        overflow-hidden flex items-center justify-center
                        font-cormorant text-4xl md:text-[3rem] font-bold text-white"
                 style="background:linear-gradient(135deg,var(--rose) 0%,var(--rose-dark) 100%);
                        box-shadow:0 20px 56px var(--rose-glow);
                        border:2px solid rgba(232,67,106,.3)">
                @if($portada)
            ...
            {{-- Status dot --}}
            <div class="absolute -bottom-1.5 -right-1.5 md:bottom-1.5 md:right-1.5
                        w-7 h-7 rounded-full flex items-center justify-center
                        text-xs font-black"
                 style="border:2.5px solid var(--bg);
                        {{ $promo ? 'background:var(--success);color:#002a1a' : 'background:rgba(255,255,255,.12);color:var(--text-3)' }}">
                {{ $promo ? '✓' : '·' }}
            </div>
        </div>
        ...
        <div class="pb-0 md:pb-6 text-center md:text-left">
            ...
            <div class="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-3">
                @if($promo)
                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[.7rem] font-bold uppercase tracking-[.05em]"
                          style="background:var(--gold-soft);border:1px solid rgba(212,168,83,.22);color:var(--gold)">
                        ⭐ Verificada
                    </span>
                @else
                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[.7rem] font-bold uppercase tracking-[.05em]"
            ...
            {{-- Nombre --}}
            <h1 class="hero-name-grad font-cormorant font-bold leading-[1.05] mb-3"
                style="font-size:clamp(2rem,5.5vw,3.2rem)">
            ...
            @if($perfil->precio)
                <div class="inline-flex items-baseline gap-2 px-4 py-2 rounded-xl"
                     style="background:var(--success-bg);border:1px solid rgba(16,217,138,.18)">
                    <span class="text-[.68rem] font-bold uppercase tracking-[.08em]" style="color:var(--text-3)">Tarifa</span>
                    <span class="font-cormorant text-[2rem] font-bold leading-none" style="color:var(--success)">
    ...
    <div class="hero-line max-w-5xl mx-auto mt-7"></div>
</div>

{{-- ═══════════════════════════
     BODY
═══════════════════════════ --}}
<div class="max-w-5xl mx-auto px-5 md:px-16 py-6 md:py-8
            grid grid-cols-1 gap-5
            md:grid-cols-[1fr_285px] md:items-start">

    {{-- ── Columna principal ── --}}
    <div class="flex flex-col gap-5 order-2 md:order-1">

        {{-- Descripción --}}
        <div class="card-accent relative rounded-[14px] p-5 md:p-6 transition-all duration-200 fade-up delay-1"
             style="background:var(--surface);border:1px solid var(--border);color:inherit">
            <div class="flex items-center gap-2 pb-3 mb-4 font-cormorant text-xl font-bold"
                 style="border-bottom:1px solid var(--border);color:var(--text)">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--rose);flex-shrink:0">
        ...
        <div class="card-accent relative rounded-[14px] p-5 md:p-6 transition-all duration-200 fade-up delay-2"
            ...
            <div class="flex items-center gap-2 pb-3 mb-4 font-cormorant text-xl font-bold"
                ...
                Galería
                <span class="text-xs font-normal ml-1" style="color:var(--text-3);font-family:'Outfit',sans-serif">
        ...
        <div class="card-accent relative rounded-[14px] p-5 md:p-6 transition-all duration-200 fade-up delay-2"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="rounded-[10px] px-4 py-3" style="background:var(--surface-2);border:1px solid var(--border)">
                    <div class="text-[.65rem] font-bold uppercase tracking-widest mb-1" style="color:var(--text-3)">Nombre</div>
                    <div class="text-sm font-semibold" style="color:var(--text)">{{ $perfil->nombre_usuario }}</div>
                </div>
                <div class="rounded-[10px] px-4 py-3" style="background:var(--surface-2);border:1px solid var(--border)">
                    <div class="text-[.65rem] font-bold uppercase tracking-widest mb-1" style="color:var(--text-3)">Edad</div>
                    <div class="text-sm font-semibold" style="color:var(--text)">{{ $perfil->edad ? $perfil->edad.' años' : 'No indicada' }}</div>
                </div>
                <div class="rounded-[10px] px-4 py-3" style="background:var(--surface-2);border:1px solid var(--border)">
                    <div class="text-[.65rem] font-bold uppercase tracking-widest mb-1" style="color:var(--text-3)">Ciudad</div>
                    <div class="text-sm font-semibold" style="color:var(--text)">{{ $perfil->ciudad ?? 'No indicada' }}</div>
                </div>
                <div class="rounded-[10px] px-4 py-3" style="background:var(--surface-2);border:1px solid var(--border)">
                    <div class="text-[.65rem] font-bold uppercase tracking-widest mb-1" style="color:var(--text-3)">Tarifa</div>
                    <div class="text-sm font-bold" style="color:var(--success)">
                        {{ $perfil->precio ? 'S/. '.number_format($perfil->precio,0) : 'Consultar' }}
                    </div>
                </div>
                @if($perfil->numero_celular)
                <div class="sm:col-span-2 rounded-[10px] px-4 py-3" style="background:var(--surface-2);border:1px solid var(--border)">
                    <div class="text-[.65rem] font-bold uppercase tracking-widest mb-1" style="color:var(--text-3)">Contacto directo</div>
    ...
    {{-- ── Columna contacto (arriba en móvil) ── --}}
    <div class="flex flex-col gap-4 order-1 md:order-2 float-in delay-1">

        <div class="rounded-[14px] p-5 md:sticky md:top-20"
             style="background:rgba(232,67,106,.06);border:1px solid var(--border-rose)">

            <h3 class="font-cormorant text-xl font-bold text-center mb-4" style="color:var(--text)">
                Contactar a {{ $perfil->nombre_usuario }}
            </h3>

            {{-- Precio --}}
            @if($perfil->precio)
            <div class="text-center rounded-xl p-3 mb-4" style="background:var(--success-bg);border:1px solid rgba(16,217,138,.15)">
                <div class="text-[.68rem] font-bold uppercase tracking-[.08em] mb-0.5" style="color:var(--text-3)">Tarifa</div>
                <div class="font-cormorant text-[2.4rem] md:text-[2.6rem] font-bold leading-none" style="color:var(--success)">
                    S/. {{ number_format($perfil->precio, 0) }}
                </div>
            ...
            @if($perfil->numero_celular)
                ...
                <a href="{{ $wa }}" target="_blank" rel="noopener"
                   class="flex items-center justify-center gap-2.5 w-full py-3.5 rounded-[11px] font-bold text-base text-white no-underline transition-all mb-3"
                   style="background:var(--wa);box-shadow:0 6px 22px rgba(37,211,102,.25)">
                ...
                <a href="tel:{{ preg_replace('/[^0-9+]/', '', $perfil->numero_celular) }}"
                   class="flex items-center justify-center gap-2 w-full py-3 rounded-[11px] font-semibold text-sm no-underline transition-all"
                ...
                <p class="text-center text-[.72rem] leading-relaxed mt-3" style="color:var(--text-3)">
...
<div class="max-w-5xl mx-auto px-5 md:px-16 pb-12 md:pb-16">
    <h2 class="font-cormorant text-[1.5rem] font-bold flex items-center gap-4 mb-5" style="color:var(--text)">
        También disponibles
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
        @foreach($sugeridos as $s)
            ...
            <a href="{{ route('perfil.publico', $s->nombre_usuario) }}"
               class="block no-underline rounded-[14px] p-4 transition-all duration-200 hover:-translate-y-1"
               style="background:var(--surface);border:1px solid var(--border);color:inherit">

                <div class="w-12 h-12 rounded-xl overflow-hidden mx-auto mb-3
                ...
                <div class="font-cormorant text-base font-bold text-center truncate" style="color:var(--text)">
                    {{ $s->nombre_usuario }}
                </div>
                ...
                <div class="text-center font-bold text-sm mt-2" style="color:var(--success)">
    ...
    <div class="flex flex-wrap gap-5">
        ...
        @guest
            <a href="{{ route('register') }}"
               ...
               style="color:var(--rose)">Publicar mi perfil →</a>
        @endguest
    </div>
